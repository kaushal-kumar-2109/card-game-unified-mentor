...
const checkCards = (cardid)=>{
    let flag=checkSelectedCard(cardid);
    // if the card selected is first card 
    if(privious==false && flag==false){
        privious=cardid;
    }
        ...
        if(flag==true){
            ...
            // if the new card is selected
                ...
                if(privious.getAttribute("name")==cardid.getAttribute("name")){
                    selectedCard.push(privious.getAttribute("id"));
                    selectedCard.push(cardid.getAttribute("id"));
                    movecount++;
...
const checkSelectedCard = (cardid)=>{
    for(let i=0;i<selectedCard.length;i++){
        if(String(selectedCard[i])==String(cardid.getAttribute("id"))){
            return true;
        }
    }
    return false;
}
//   -------------------------------------------  checkSelectedCard function end !   -------------------------------------------
